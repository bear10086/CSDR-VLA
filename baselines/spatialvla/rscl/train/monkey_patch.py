...
from train.csdr_loss import CSDRConfig, prompt_token_signature, spatialvla_csdr_loss
from train.rscl_loss import rscl_loss, rscl_embedding_loss, _base_model
...
def configure_rscl_trainer(
    ...
    """Attach the RS-CL representation path to the same final hidden state hook."""
    trainer.rscl_beta = float(beta)
    trainer.rscl_temperature = float(temperature)
    trainer.rscl_lambda_init = float(lambda_init)
    trainer.rscl_task_ratio = float(task_ratio)
    trainer.rscl_warmup_steps = int(warmup_steps)
    trainer.rscl_total_steps = max(int(trainer.args.max_steps), 1)
    trainer._rscl_last_logged_step = -1
    unwrapped = trainer.accelerator.unwrap_model(trainer.model)
    candidates = [
        module for name, module in unwrapped.named_modules()
        if name.endswith("language_model.model.norm")
    ]
    if len(candidates) != 1:
        names = [name for name, _ in unwrapped.named_modules() if name.endswith(".norm")]
        raise RuntimeError(
            f"Expected one language model final norm for RS-CL, found {len(candidates)}; "
            f"norms={names[-10:]}"
        )

    def capture_last_hidden(_module, _inputs, output):
        trainer._csdr_hidden_cache["last_hidden"] = output
        attention_mask = getattr(trainer, "_rscl_attention_mask", None)
        if attention_mask is None:
            return
        base = _base_model(unwrapped)
        required = ("rscl_summary_token", "rscl_adapter", "rscl_projector", "rscl_dropout")
        if not all(hasattr(base, name) for name in required):
            return
        mask = attention_mask.to(output.device, dtype=output.dtype).unsqueeze(-1)
        denom = mask.sum(dim=1).clamp_min(1.0)
        pooled = ((output * mask).sum(dim=1) / denom).float()
        ...
        trainer._csdr_hidden_cache["rscl_z"] = torch.nn.functional.normalize(z, dim=-1, eps=1e-6)
        trainer._csdr_hidden_cache["rscl_z_aug"] = torch.nn.functional.normalize(z_aug, dim=-1, eps=1e-6)

    trainer._csdr_hidden_cache = {}
    trainer._rscl_attention_mask = None
    trainer._rscl_hook_handle = candidates[0].register_forward_hook(capture_last_hidden)

    def expose_auxiliary_outputs(_module, _inputs, outputs):
        # DDP's unused-parameter traversal starts from the returned tensors.
        # A Python side cache alone is not visible to that traversal.
        if trainer._rscl_attention_mask is not None:
            # ModelOutput dataclasses cannot reconstruct undeclared fields
            # when DDP unflattens its output tree. A dict preserves all fields.
            outputs = dict(outputs)
            outputs["rscl_z"] = trainer._csdr_hidden_cache.pop("rscl_z")
            outputs["rscl_z_aug"] = trainer._csdr_hidden_cache.pop("rscl_z_aug")
        return outputs

    trainer._rscl_output_hook_handle = unwrapped.register_forward_hook(expose_auxiliary_outputs)
...
def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
    ...
    # Proprioception is an auxiliary RS-CL target and is not a model-forward argument.
    proprio = inputs.pop("proprio", None)
    self._rscl_attention_mask = inputs.get("attention_mask") if hasattr(self, "rscl_beta") else None
    outputs = model(**inputs)
    self._rscl_attention_mask = None
    ...
    if hasattr(self, "rscl_beta"):
        if "last_hidden" not in self._csdr_hidden_cache:
            raise RuntimeError("RS-CL last-hidden hook did not run.")
        last_hidden = self._csdr_hidden_cache.pop("last_hidden")
        if "rscl_z" not in outputs:
            raise RuntimeError("RS-CL projected embeddings were not captured in the forward hook.")
        rscl = rscl_embedding_loss(
            z=outputs["rscl_z"],
            z_aug=outputs["rscl_z_aug"],
            proprio=proprio,
            beta=self.rscl_beta,
            temperature=self.rscl_temperature,
        )
        raw_aux = rscl["rscl_loss"]
        step = int(self.state.global_step)
        progress = min(max(step / max(self.rscl_total_steps - 1, 1), 0.0), 1.0)
        schedule = 0.5 * (1.0 + float(torch.cos(torch.tensor(progress * torch.pi)).item()))
        if self.rscl_warmup_steps > 0:
            schedule *= min((step + 1) / self.rscl_warmup_steps, 1.0)
        budget = task_loss.detach().abs() * self.rscl_task_ratio * schedule
        raw_scaled = raw_aux * self.rscl_lambda_init
        aux_scale = torch.minimum(
            raw_scaled.detach().new_tensor(1.0),
            budget / raw_scaled.detach().abs().clamp_min(1e-8),
        )
        contribution = aux_scale * raw_scaled
        loss = task_loss + contribution
        metrics.update({name: value.detach() for name, value in rscl.items()})
        metrics.update({
            "rscl_contribution": contribution.detach(),
            "rscl_scale": aux_scale.detach(),
            "rscl_schedule": contribution.detach().new_tensor(schedule),
            "rscl_to_task_ratio": contribution.detach().abs()
                / task_loss.detach().abs().clamp_min(1e-8),
        })
    elif hasattr(self, "csdr_config"):
        if "last_hidden" not in self._csdr_hidden_cache:
        ...
        budget = task_loss.detach().abs() * self.csdr_order_to_task_ratio * schedule
        order_scale = torch.minimum(
            raw_order.detach().new_tensor(1.0),
            budget / raw_order.detach().abs().clamp_min(1e-8),
        )
        contribution = order_scale * raw_order
        loss = task_loss + contribution
