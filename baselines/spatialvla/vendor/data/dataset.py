...
class OpenXIterableDataset(IterableDataset):
    ...
    def __init__(
        ...
        # TFDS exports may already include exact full-dataset statistics. Reuse
        # them explicitly so a code-version cache hash cannot trigger a costly
        # second scan over every trajectory.
        for dataset_kwargs in per_dataset_kwargs:
            ...
            if len(statistics_files) == 1:
                dataset_kwargs["dataset_statistics"] = statistics_files[0]
                print(f"[Dataset] reuse statistics: {statistics_files[0]}")
        ...
        self.rlds_config = dict(
            ...
            train=self.is_train,
            shuffle_seed=self._shuffle_seed_for_rank(),
        )        
        ...
        # The interleaver receives the loaded statistics separately. Keeping the
        # cache path in each dataset kwargs would pass the argument twice.
        for dataset_kwargs in self.rlds_config["dataset_kwargs_list"]:
    ...
    def _shuffle_seed_for_rank(self):
        cohort_count = int(os.environ.get("CSDR_PROMPT_COHORTS", "0"))
        if cohort_count <= 0:
            return 3407 * self.current_rank
        cohort_count = min(cohort_count, self.total_ranks)
        cohort_index = self.current_rank * cohort_count // self.total_ranks
        return 3407 * (cohort_index + 1)
    ...
    def reset_rlds_iterator(self):
        """Rebuild the finite RLDS stream when a training wrapper starts a new pass."""
        self.rlds_dataset = None
